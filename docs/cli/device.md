...
don't pass it every time. `anvil device` on its own does nothing; run one of its
subcommands.
...
```bash
anvil device <subcommand>
```

## Prerequisites

- You're [signed in](/cli/login) (`anvil login`) with an [organization](/cli/org)
  selected (`anvil org select`)
...
```bash
anvil device select "Living Room Controller"
```
