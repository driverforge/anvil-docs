---
sidebar_position: 1
description: How your project's driver token works, why it's safe to commit, and how it differs from a secret API key.
---
...
The rest of this page is about your driver token, which is what you need to send telemetry. Organization-level secret API keys are still in development; you'll find the placeholder under **Settings → API Keys**.
...
A driver token **identifies**; it doesn't authenticate or authorise. It answers "which project is this telemetry for?", the same job an `{id}` segment does in a URL. Something else proves who is sending: the agent holds a signed token from Anvil's identity server, the broker verifies it on connect, and Anvil then checks that the sender has permission on the project's organisation. Your driver token is not what gets you in.
...
:::tip
Don't build a pipeline to keep your driver token out of source control. It isn't a secret; commit it like any other configuration value.
:::

## What a driver token can do

Driver tokens are **write-only**. A token names exactly one project, and telemetry sent with it lands in that project. It **cannot**:
...
Driver tokens begin with `drv_`, followed by 27 characters:
...
- `dfa_…` and `dfs_…` are secret organization API keys. Anything starting `df` is a secret, always. These are stored hashed and shown only once, so they behave nothing like a driver token.

## Your driver token doesn't expire

A project has **one** driver token, for its whole life. There's nothing to rotate, expire or revoke, and no list of old tokens to keep track of.

Open **Settings → Projects**, click the **gear icon** on the project's row, and find the **Driver Token** section in the drawer that opens. Copy it whenever you need it — the full value stays available, because it isn't a secret.
