---
sidebar_position: 1
description: Run the anvil CLI in your CI/CD pipeline with the GitHub Action or Buildkite plugin.
---

# Overview

Run `anvil` in your pipeline to build and ship your Control4 drivers automatically
...
:::info Preview
The `anvil` GitHub Action and Buildkite plugin are in **preview**. This section
...
Both integrations follow the same model: **they install a pinned `anvil` release
...
Omit the command and the integration simply makes `anvil` available, so you can
...
Nothing extra on the runner. `anvil` is a self-contained native binary, so the
